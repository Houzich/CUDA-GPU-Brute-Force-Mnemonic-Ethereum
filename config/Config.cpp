...
int parse_config(ConfigClass* config, std::string path)
{
	try {
		...
		config->folder_save_result = access(v, tao::config::key("folder_save_result")).get_string();	
		config->lines_in_file_save = access(v, tao::config::key("lines_in_file_save")).get_unsigned();
