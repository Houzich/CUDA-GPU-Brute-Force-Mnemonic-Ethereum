...
#pragma omp critical 
	...
	void addInFileTest(int num_bytes, std::string& path, std::string& mnemonic, std::string& hash160, std::string& hash160_in_table, std::string& addr, std::string& addr_in_table) {
		...
		if (out.is_open())
		{
			...
			out << "EQUAL " << num_bytes << "," << mnemonic << ",address path " << pth << "," << addr << "," << addr_in_table << "," << std::asctime(std::localtime(&now));
		}
		else
		{
	...
	std::string getPath(uint32_t path, uint32_t child)
	{
		std::stringstream ss;
		std::string pth = "";
		if (path == 0) ss << "m/44'/60'/0'/0/" << child;
		if (path == 1) ss << "m/44'/60'/0'/1/" << child;
		return ss.str();
	}
	...
	int checkResult(retStruct* ret) {
		...
		if (ret->f[0].count_found != 0)
		{
			for (uint32_t i = 0; i < ret->f[0].count_found; i++)
			{
				...
				std::string mnemonic = (const char*)info->mnemonic;
				std::string addr;
				std::string path = getPath(info->path, info->child);
				tools::encodeAddressEthereumBIP44((const uint8_t*)info->hash160, addr);
		...
		if (ret->f[0].count_found_bytes != 0)
		{
			for (uint32_t i = 0; i < ret->f[0].count_found_bytes; i++)
			{
				...
				std::string path = getPath(info->path, info->child);
				tools::encodeAddressEthereumBIP44((const uint8_t*)info->hash160, addr);
				tools::encodeAddressEthereumBIP44((const uint8_t*)info->hash160_from_table, addr_in_table);
				std::cout << "\n!!!FOUND IN ADDRESS(HASH160) (" << path << ") EQUAL " << num_bytes << " BYTES: " << mnemonic << "," << addr << "," << addr_in_table << " \n";
