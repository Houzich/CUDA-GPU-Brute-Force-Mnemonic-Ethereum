...
namespace tools {
	...
	int encodeAddressEthereumBIP44(const uint8_t* hash160, std::string& addr)
	{
		...
		addr = "0x" + bytesToHexString(hash160, 20);
	...
	std::string wordsIndexToMnemonic(const uint16_t* words_index)
	{
		...
		for (int i = 0; i < 11; i++) {
			...
			mnemonic += words[word_index];
			mnemonic += ' ';
		}
		int word_index = words_index[11];
		mnemonic += words[word_index];
		return mnemonic;
	}
