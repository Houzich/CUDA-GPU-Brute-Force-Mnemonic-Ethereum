...
__device__
int find_hash_in_table(const uint32_t* hash, const tableStruct table, const uint32_t* mnemonic, retStruct* ret, int num_bytes) {
	int found = 0;
	...
	while (point < line_cnt) {
		...
		if (cmp <= 0) {
			if (cmp == 0)
			{
				...
				for (int i = 0; i < SIZE32_MNEMONIC_FRAME; i++) ret->mnemonic[i] = mnemonic[i];
			}
			//printf("exit cmp = %d\n", cmp);
			break;
		}
		...
		if (found == 2) {
				...
				{
					ret->flag_found_bytes = found;
					for (int i = 0; i < 5; i++)
					{
						ret->hash160_bytes_from_table[i] = hash_from_table[i];
						ret->hash160_bytes[i] = hash[i];
					}
					for (int i = 0; i < SIZE32_MNEMONIC_FRAME; i++) ret->mnemonic_bytes[i] = mnemonic[i];
				}
...
)
{
	...
	int found = find_hash_in_table(hash160_buffer, tables[num_tables], mnemonic, ret, num_bytes_find[0]);
...
)
{
	...
	for (int i = 0; i < NUM_CHILDS; i++) {
		...
		dev_search_hash160_in_tables(hash, tables, mnemonic, ret);
	}
	//______________________________________________________________________________________________________________________
	//______________________________________________________________________________________________________________________
	//______________________________________________________________________________________________________________________
...
)
{
	...
	for (int num_cld = 0; num_cld < NUM_CHILDS; num_cld++) {
		normal_private_child_from_private(&target_key, &target_key_fo_pub, num_cld);
		calc_public(&target_key_fo_pub, &target_public_key);
		calc_hash160((uint32_t*)&target_public_key.key, &hash[(num_cld + NUM_CHILDS * 0) * 5]);
		dev_search_hash160_in_tables(&hash[(num_cld + NUM_CHILDS * 0) * 5], tables, mnemonic, ret);
	}
	//______________________________________________________________________________________________________________________
	//______________________________________________________________________________________________________________________
	//______________________________________________________________________________________________________________________
	...
	for (int num_cld = 0; num_cld < NUM_CHILDS; num_cld++) {
		normal_private_child_from_private(&target_key, &target_key_fo_pub, num_cld);
		calc_public(&target_key_fo_pub, &target_public_key);
		calc_hash160((uint32_t*)&target_public_key.key, &hash[(num_cld + NUM_CHILDS * 1) * 5]);
		dev_search_hash160_in_tables(&hash[(num_cld + NUM_CHILDS * 1) * 5], tables, mnemonic, ret);
	}
}
...
)
{
	...
	for (int x = 0; x < 120 / 4; x++) {
		ipad[x] = 0x36363636 ^ *(uint32_t*)((uint32_t*)mnemonic + x);
		opad[x] = 0x5C5C5C5C ^ *(uint32_t*)((uint32_t*)mnemonic + x);
	}
	...
	for (int x = 0; x < 12 / 4; x++) {
		ipad[x + 128 / 4] = *(uint32_t*)((uint32_t*)&salt + x);
	}
	ipad[140 / 4] = 0;
	opad[192 / 4] = 0;
	sha512((uint64_t*)ipad, 140, (uint64_t*)&opad[128 / 4]);
	sha512((uint64_t*)opad, 192, (uint64_t*)&ipad[128 / 4]);
	XOR_SEED_WITH_ROUND_32(seed, (&ipad[128 / 4]));
	...
	for (int x = 0; x < 12 / 4; x++) {
		ipad[x] = 0x36363636 ^ *(uint32_t*)((uint32_t*)&key + x);
		opad[x] = 0x5C5C5C5C ^ *(uint32_t*)((uint32_t*)&key + x);
	}

	for (int x = 12 / 4; x < 128 / 4; x++) {
		ipad[x] = 0x36363636;
		opad[x] = 0x5C5C5C5C;
	}
...
)
{
	...
	key_to_hash160_for_save((extended_private_key_t*)&ipad[128 / 4], tables, (uint32_t*)mnemonic, ret, hash);
	...
	for (int i = 0; i < NUM_ALL_CHILDS; i++)
	{
		...
		*save++ = 'x';
		...
		{
			*save++ = smblhex[(hash8[i * 20 + ii]) >> 4];
			*save++ = smblhex[(hash8[i * 20 + ii]) & 0x0F];
		}
		*save++ = ',';
	}
	*(save - 1) = '\n';
